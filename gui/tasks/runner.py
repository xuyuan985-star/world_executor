...
# ---- 退出保护注册表（0.6.0 回滚后保留：HealthWorker 等 QThread） ----
# aboutToQuit 时若有 QThread 仍在运行（check_health 加载 OCR 模型 1-2s），
# Qt 析构运行中 QThread → 0xC0000409 崩溃。创建即注册，guard 只查注册表。
# m7 任务在独立 QProcess（非 QThread）——不在此列。
import threading
...
def register_qthread(t):
    with _ALL_QTHREADS_LOCK:
        _ALL_QTHREADS.add(t)


def unregister_qthread(t):
    with _ALL_QTHREADS_LOCK:
        _ALL_QTHREADS.discard(t)


def all_running_qthreads():
    with _ALL_QTHREADS_LOCK:
